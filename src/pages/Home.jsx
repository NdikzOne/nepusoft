...
const Home = () => {
        ...
        useEffect(() => {
            ...
            ongoingCardRefs.current.forEach((ref) => {
                if (ref) observer.observe(ref);
            });
            todayCardRefs.current.forEach((ref) => {
                if (ref) observer.observe(ref);
            });
            popularCardRefs.current.forEach((ref) => {
                if (ref) observer.observe(ref);
            });
      ...
      `
                ...
                header className = "relative w-full aspect-[16/10] md:aspect-video min-h-[300px] md:max-h-[550px] overflow-hidden bg-[#0f0f12]" > {
                    isLoading ? < HeroSkeleton / > : ( <
                        div className = {
                            `flex h-full ${isTransitioning ? 'transition-transform duration-700' : ''}`
                        }
                        style = {
                            {
                            ...
                            carouselItems.map((a, i) => ( <
                                ...
                                <
                                img src = {
                                    `https://proxy-phi-two-35.vercel.app/proxy?url=${encodeURIComponent(
    a.image_cover || a.image_poster
  )}`
                                }
      ...
      a.image_poster || a.image_cover
                                ...
                                <
                                h2 className = "text-lg md:text-3xl font-black text-white tracking-tight leading-tight line-clamp-2" > {
                                    a.title
                                } < /h2> <
                                ...
                                <
                                svg className = "w-3.5 h-3.5 md:w-4 md:h-4 fill-current"
                    ...
                    !isLoading && todayAnime.length > 0 && ( <
                        ...
                        span className = "text-[10px] md:text-xs font-black text-white" > {
                            (heroIndex % todayAnime.length) + 1
                        }
                        / {todayAnime.length}</span >
                        <
                        div className = "w-8 md:w-10 h-[2px] bg-white/20 rounded-full" > < /div> <
                        button onClick = {
                            () => {
                                if (isTransitioning && heroIndex < todayAnime.length) setHeroIndex(p => p + 1);
                            }
                        }
                ...
                <
                img src = "https://raw.githubusercontent.com/alip-jmbd/alipp/main/bc.jpg"
                ...
                className = "bg-white/5 hover:bg-[#F6CF80] hover:text-black hover:border-[#F6CF80] border border-white/10 px-4 py-2 rounded-lg flex items-center gap-2 transition-all font-black text-[11px] text-white" >
                <
                svg className = "w-3.5 h-3.5"
                fill = "none"
                stroke = "currentColor"
                viewBox = "0 0 24 24"
                strokeWidth = "2.5" > < path strokeLinecap = "round"
                strokeLinejoin = "round"
                ...
                className = "bg-[#1877F2]/10 hover:bg-[#1877F2] border border-[#1877F2]/20 px-4 py-2 rounded-lg flex items-center gap-2 transition-all group" >
                <
                svg className = "w-3.5 h-3.5 fill-[#1877F2] group-hover:fill-white transition-colors"
                viewBox = "0 0 24 24" > < path d = "M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" / > < /svg> < /
                button > <
                button onClick = {
                    () => handleShare('x')
                }
                className = "bg-white/5 hover:bg-white border border-white/10 px-4 py-2 rounded-lg flex items-center gap-2 transition-all group" >
                <
                svg className = "w-3.5 h-3.5 fill-white group-hover:fill-black transition-colors"
                viewBox = "0 0 24 24" > < path d = "M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" / > < /svg> < /
                button > <
                button onClick = {
                    () => handleShare('tg')
                }
                className = "bg-[#229ED9]/10 hover:bg-[#229ED9] border border-[#229ED9]/20 px-4 py-2 rounded-lg flex items-center gap-2 transition-all group" >
                <
                svg className = "w-3.5 h-3.5 fill-[#229ED9] group-hover:fill-white transition-colors"
                ...
                className = "bg-white/5 hover:bg-white hover:text-black border border-white/10 px-4 py-2 rounded-lg flex items-center gap-2 transition-all font-black text-[11px] text-white" >
                <
                svg className = "w-3.5 h-3.5"
                fill = "none"
                stroke = "currentColor"
                ...
                svg className = "w-5 h-5 text-white/40 group-hover:text-[#F6CF80] transition-colors"
                fill = "none"
                stroke = "currentColor"
                strokeWidth = "3"
                viewBox = "0 0 24 24" > < path strokeLinecap = "round"
                ...
                <
                button onClick = {
                    () => scroll(ongoingScrollRef, 'left')
                }
                className = "w-8 h-8 flex items-center justify-center bg-white/5 border border-white/10 rounded-full group hover:bg-white/20 transition-colors" > < svg className = "w-4 h-4 text-white/50 group-hover:text-white"
                viewBox = "0 0 24 24"
                fill = "none"
                stroke = "currentColor"
                ...
                className = "flex overflow-x-auto gap-3 pb-4 custom-scrollbar snap-x px-2" > {
                    isLoading ? [...Array(8)].map((_, i) => < CardSkeleton key = {
                            i
                        }
                        />) : 
                        ongoing.map((a, i) => ( <
                            ...
                            ref = {
                                el => ongoingCardRefs.current[i] = el
                            }
                            onClick = {
                                () => navigate(`/anime/${a.id}-${(a.title||'').toLowerCase().replace(/[^a-z0-9]+/g, '-')}`)
                            }
                    ...
                    className = "flex overflow-x-auto gap-3 pb-4 custom-scrollbar snap-x px-2" > {
                        ...
                        div className = "grid grid-cols-1 md:grid-cols-2 gap-4 px-2" > {
                            isLoading ? [...Array(10)].map((_, i) => < div key = {
                                    i
                                }
                                className = "h-24 bg-[#16161a] rounded-xl relative overflow-hidden" > < Shimmer / > < /div>) :
                                popular.slice(0, 10).map((anime, index) => ( <
                                    div key = {
                                        anime.id
                                    }
                                    ref = {
                                        el => popularCardRefs.current[index] = el
                                    }
                                    onClick = {
                                        () => navigate(`/anime/${anime.id}-${(anime.title||'').toLowerCase().replace(/[^a-z0-9]+/g, '-')}`)
                                    }
                                    className = {
                                    ...
                                    <
                                    div className = {
                                        `w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center font-black text-sm md:text-base shrink-0 shadow-lg ${index < 3 ? 'bg-[#F6CF80] text-[#0a0a0c]' : 'text-white/30 border border-white/10 bg-white/5'}`
                                    } > {
                                        index + 1
                                    } < /div> <
                                    div className = "flex flex-col" >
                                    <
                                    h3 className = "text-white font-bold text-sm md:text-base line-clamp-1 group-hover:text-[#F6CF80] transition-colors" > {
                                        anime.title
                                    } < /h3> < /
                                    div > <
                                    /div> < /
